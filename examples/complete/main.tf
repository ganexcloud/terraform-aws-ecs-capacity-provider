...
module "security_group_ecs_cluster" {
  ...
  vpc_id              = module.vpc_ecs-production.vpc_id
...
module "autoscalinggroup_nodes_staging" {
  ...
  key_name                  = "vpn"
  security_groups           = [module.security_group_ecs-cluster.security_group_id]
  iam_instance_profile_name = module.iam_assumable_role_production-ecsinstancerole.iam_instance_profile_id
  user_data                 = <<-EOT
  #!/bin/bash
  echo "ECS_CLUSTER=${"production"}" >> /etc/ecs/ecs.config
  EOT
  vpc_zone_identifier       = module.vpc_ecs-production.public_subnets
...
module "ecs_capacity_provider" {
  ...
  version                   = "0.0.1"
  name                      = "nodes-staging"
  auto_scaling_group_arn    = module.autoscalinggroup_nodes-staging.autoscaling_group_arn
  target_capacity           = 100
